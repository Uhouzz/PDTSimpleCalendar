...
#import "UIView+PDTRoundedCorners.h"
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat calendarCircleWidth = floorf(CGRectGetWidth([UIScreen mainScreen].bounds)/7);
        
        //Add the Constraints
        [self.dayLabel setTranslatesAutoresizingMaskIntoConstraints:NO];
        [self.dayLabel setBackgroundColor:[UIColor clearColor]];
//        self.dayLabel.layer.cornerRadius = calendarCircleSize/2;
//        self.dayLabel.layer.masksToBounds = YES;
        ...
        [self.contentView addConstraint:[NSLayoutConstraint constraintWithItem:self.dayLabel attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:calendarCircleWidth -2]];
        [self.contentView addConstraint:[NSLayoutConstraint constraintWithItem:self.dayLabel attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:calendarCircleWidth]];
...
- (void)setIsStarday:(BOOL)isStarday
{
    _isStarday = isStarday;
    
}

- (void)setIsEndday:(BOOL)isEndday{
    _isEndday = isEndday;
}

- (void)setIsSameday:(BOOL)isSameday {
...
- (void)setRoundedCorners{
    CGFloat calendarCircleWidth = floorf(CGRectGetWidth([UIScreen mainScreen].bounds)/7);
    CGSize  circleSize = CGSizeMake((calendarCircleWidth/2), (calendarCircleWidth/2));
    
    [self.dayLabel removeRoundedCorners];
    
    if (self.isSameday) {
        [self.dayLabel addRoundedCorners:UIRectCornerAllCorners withRadii:circleSize];
    }else{
        if (self.isStarday) {
            [self.dayLabel addRoundedCorners:UIRectCornerTopLeft| UIRectCornerBottomLeft withRadii:circleSize];
        }
        
        if (self.isEndday) {
            [self.dayLabel addRoundedCorners:UIRectCornerTopRight| UIRectCornerBottomRight withRadii:circleSize];
        }
    }
